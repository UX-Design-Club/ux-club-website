...
export function PrimaryButton({ href, text }: { href: string, text: string }) {
  ...
  return (
    ...
    >
      ...
      <span className={`relative flex flex-row items-center font-bold uppercase mx-2 transition-color text-slate-200 group-hover:text-black duration-${transitionLength}`}>
        {text}
      </span>
    </Link>
  );
